...
def feature_rasterization(
    means: Tensor,  # [N, 3]
    quats: Tensor,  # [N, 4]
    scales: Tensor,  # [N, 3]
    opacities: Tensor,  # [N]
    colors: Tensor,  # [(C,) N, D] or [(C,) N, K, 3]
    features: Tensor, # [(C, ) N, D]
    ...
    feature_backgrounds: Optional[Tensor] = None,
    ...
    Args:
        ...
        feature_backgrounds: The background feature. [C, D]. Default is None.
    ...
    # treat features as post-activation values, should be in shape [N, D] or [C, N, D]
    assert (features.dim() == 2 and features.shape[0] == N) or (
        features.dim() == 3 and features.shape[:2] == (C, N)
    ), features.shape
    ...
    # print(f"{colors.shape} cat {features.shape}")
    colors = torch.cat((colors, features), dim=-1)
    D = features.shape[-1] if features is not None else 0
    if backgrounds is not None:
        if feature_backgrounds is not None:
            assert (feature_backgrounds.shape[-1] == D)
            backgrounds = torch.cat(
                [backgrounds, feature_backgrounds], dim=-1
            )
        else:
            backgrounds = torch.cat(
                [backgrounds, torch.zeros(C, D, device=backgrounds.device)], dim=-1
            )  
    # Rasterize to pixels
...
def main():
    ...
    # 外参矩阵 (View Matrix) [C, 4, 4]
    # 我们假设相机在原点，Identity 矩阵
    ...
    # ==========================================
    num_gaussians = 1
    # 位置：放置在相机正前方 Z=3.0 的位置
    xyz = torch.tensor([[0.0, 0.0, 3.0]], dtype=torch.float32, device=device)
    # 旋转：无旋转 (四元数 w,x,y,z)
    rot = torch.tensor([[1.0, 0.0, 0.0, 0.0]], dtype=torch.float32, device=device)
    # 缩放：设置一个相对较大的尺寸，方便观察
    scale = torch.tensor([[0.5, 0.5, 0.5]], dtype=torch.float32, device=device)
    # 不透明度：完全不透明
    opacity = torch.tensor([1.0], dtype=torch.float32, device=device)

    # ==========================================
    # 3. 初始化颜色 (红色，转换为球谐系数)
    # ==========================================
    sh_degree = 3
    num_sh_bases = (sh_degree + 1) ** 2  # 16
    color = torch.zeros((num_gaussians, num_sh_bases, 3), dtype=torch.float32, device=device)
    
    # 3DGS标准中：RGB = SH_C0 * sh_dc + 0.5，因此 sh_dc = (RGB - 0.5) / SH_C0
    SH_C0 = 0.28209479177387814
    red_rgb = torch.tensor([1.0, 0.0, 0.0], device=device)
    color[:, 0, :] = (red_rgb - 0.5) / SH_C0  # 仅修改 0 阶 DC 分量，高阶设为 0

    # ==========================================
    # 4. 初始化 Features
    # ==========================================
    # 6个通道：前三个通道是蓝色 [0,0,1]，后三个通道是绿色 [0,1,0]
    features = torch.tensor([[0.0, 0.0, 1.0, 0.0, 1.0, 0.0]], dtype=torch.float32, device=device)

    print(f"{color.shape} and {features.shape}")

    render_colors, render_alphas, info = feature_rasterization(
        means=xyz,
        quats=rot,
        scales=scale,
        opacities=opacity,
    ...
    rendered_img = render_colors[0] 
    rgb_map = rendered_img[..., 0:3].cpu().numpy()
    feature_map1 = rendered_img[..., 3:6].cpu().numpy()
    feature_map2 = rendered_img[..., 6:9].cpu().numpy()
    depth_map = rendered_img[..., 9:10].cpu().numpy()
